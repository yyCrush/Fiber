 ...
 */
#ifndef __myfiber_CONFIG_H__
#define __myfiber_CONFIG_H__
...
// @brief 类型转换模板类片特化(YAML的String 转换成 std::vector<T>)
template<class T>
...
public:
    std::string operator()(const std::vector<T>& v) {
        ...
        for(auto& i : v) {
            //递归将T转换成string，自定义的person需要提供LexicalCast<Person, std::string>
...
public:
    ...
    std::string toString() override {
        try {
            ...
            myfiber_LOG_ERROR(myfiber_LOG_ROOT()) << "ConfigVar::toString exception "
    ...
    bool fromString(const std::string& val) override {
        try {
            ...
            myfiber_LOG_ERROR(myfiber_LOG_ROOT()) << "ConfigVar::fromString exception "
...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,
        ...
        if(it != GetDatas().end()) {
            auto tmp = std::dynamic_pointer_cast<ConfigVar<T>>(it->second);
            if (tmp) {
                myfiber_LOG_ERROR(myfiber_LOG_ROOT()) << "Lookup name=" << name << "exists";

            } else {
                myfiber_LOG_ERROR(myfiber_LOG_ROOT())<< "Lookup name=" << name << " exists but type not "
        ...
        if(name.find_first_not_of("abcdefghikjlmnopqrstuvwxyz._012345678")
                != std::string::npos) {
            myfiber_LOG_ERROR(myfiber_LOG_ROOT()) << "Lookup name invalid " << name;
